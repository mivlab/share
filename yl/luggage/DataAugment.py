...
def img_rotation(img, dsize, min_angel = 50, max_angel = 100, min_scale = 1, max_scale = 1, fill_pixel = 0) :
	img = img.astype(np.uint8)
	h, w, c = img.shape
	_angel = random.randint(min_angel, max_angel)
	_scale = random.uniform(min_scale, max_scale)
	rotation_matrix = cv2.getRotationMatrix2D((h // 2, w // 2), _angel, _scale)
	out = cv2.warpAffine(img, rotation_matrix, dsize)
	#cv2.imshow('out1', out)
	...
	if fill_pixel == 255 :
		...
		mask = cv2.warpAffine(mask, rotation_matrix, dsize)
		# 腐蚀图像
		...
		for i in range (dsize[1]) :
			for j in range(dsize[0]) :
				if mask[i, j, 0] == 0 and mask[i, j, 1] == 0 and mask[i, j, 2] == 0 :
					out[i, j, :] = 255
	return out, _angel, rotation_matrix
